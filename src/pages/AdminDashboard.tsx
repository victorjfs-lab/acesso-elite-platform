...
export default function AdminDashboard() {
  ...
  const deleteRequestMutation = useMutation({
    mutationFn: (requestId: string) => academyRepository.deleteEnrollmentRequest(requestId),
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ["admin-overview"] });
      toast({
        title: "Solicitacao removida",
        description: "O pedido foi excluido da lista de matriculas.",
      });
    },
    onError: (error) => {
      toast({
        title: "Nao foi possivel excluir a solicitacao",
  ...
  return (
    <div className="space-y-6 rounded-[2rem] bg-[#f5f8fc] p-1">
      ...
      <section className="grid gap-6 xl:grid-cols-[0.88fr_1.12fr]">
        ...
        <Card className="overflow-hidden border-slate-200 bg-white shadow-[0_12px_36px_rgba(15,23,42,0.05)]">
          <CardHeader className="border-b border-slate-100 px-4 py-3">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex flex-wrap items-center gap-1.5 text-[10px] font-semibold uppercase tracking-[0.16em] text-slate-500">
                <span className="rounded-full bg-slate-100 px-2.5 py-0.5">4 por página</span>
          ...
          <CardContent className="p-0">
            {visibleRequests.length === 0 ? (
              <div className="m-4 rounded-2xl border border-dashed border-slate-200 bg-slate-50/70 px-4 py-4 text-xs text-slate-500">
              ...
              <div className="divide-y divide-slate-100">
                {visibleRequests.map((request) => (
                  <div
                    key={request.id}
                    className="grid gap-2.5 px-4 py-2.5 transition hover:bg-slate-50/75 lg:grid-cols-[minmax(200px,1.05fr)_minmax(190px,0.9fr)_minmax(170px,0.75fr)_auto] lg:items-center"
                  >
                    <div className="flex min-w-0 items-start gap-2.5">
                      <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-slate-900 text-[11px] font-semibold text-white shadow-sm">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="mt-0.5 flex flex-wrap gap-x-2 gap-y-0.5 text-[11px] text-slate-500">
                          <span className="max-w-[210px] truncate">{request.email}</span>
                    ...
                    <div className="min-w-0 space-y-0.5">
                      ...
                      <p className="truncate text-[11px] font-medium text-primary/85">{request.linkTitle}</p>
                      {request.notes ? (
                        <p className="line-clamp-1 text-[11px] text-slate-400">{request.notes}</p>
                      ) : null}
                    </div>

                    <div className="flex flex-wrap gap-1.5 text-[10px]">
                      <span className="inline-flex items-center gap-1.5 rounded-full border border-slate-200 bg-slate-50 px-2.5 py-1">
                        <span className="font-semibold uppercase tracking-[0.14em] text-slate-400">Cadastro</span>
                    ...
                    <div className="flex flex-wrap items-center gap-1.5 lg:justify-end">
                      ...
                      {request.status === "pending" ? (
                        <>
                          <select
                            className="h-8 w-24 rounded-xl border border-slate-200 bg-white px-2.5 text-[11px] outline-none transition focus:border-primary"
                      ...
                      <Button
                        variant="outline"
                        size="icon"
                        className="h-8 w-8 rounded-xl border-red-100 text-red-600 hover:bg-red-50 hover:text-red-700"
                        disabled={deleteRequestMutation.isPending}
                        title="Excluir solicitação"
                        onClick={() => {
                          const shouldDelete = window.confirm(
                            "Excluir esta solicitação de matrícula? Esta ação não apaga o aluno nem os acessos já liberados.",
                          );
